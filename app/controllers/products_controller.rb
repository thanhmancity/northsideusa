class ProductsController < ApplicationController
  ...
  def men_sport
    @products = Product.joins(:product_colors, :product_categories).order(:name).where(gender: 'm', age_group: 'a', enabled: 1).where('product_categories.category_id = 5 AND product_colors.enabled = 1').select("products.*, product_colors.color, product_colors.id AS pcid")
    @order_item = current_order.order_items.new
  end
